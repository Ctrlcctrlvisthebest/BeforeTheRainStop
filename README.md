...
可以跳到同伴头上。找齐钥匙后，全员抵达金色灯门才通关；星星是可选挑战。机关要连续踩住 4 秒，可同时展翼挡雨；多人同时占住两块圆垫，单人只需一块。离开会清空蓄力，开启后保持开启。

四个关卡分别引入转面绕墙、台阶与合作机关、风柱与移动平台、周期雨刃。所有关卡可在大厅选择。手机提供触屏按键，横屏更适合操作。

## 挡雨与和纸

- 雨幕交替出现细雨与骤雨；湿度达到 100% 时，该纸鹤回到自己的最近存档旗，队伍收藏保留。
- 在地面按住 S / ↓ 展翼，不能同时移动、跳跃或折桥。保护半径为 2.6 个世界单位，遮住附近同伴；保护范围按真实空间计算，转面不会扩大范围。
- 展翼者自己承受原雨量的 28%，受同伴保护者每秒晾干 10%。两只纸鹤可以互相遮雨。
- 屋檐、上方实体平台以及无雨处每秒晾干 25%。能站上去的台阶下方也可避雨；跳出屋檐高度就会再次淋雨。
- 第四关可在最后机关蓄力后，到中间的檐亭晾干，再跳过最后的雨刃。单人可以靠展翼与檐亭完成所有关卡。
- 画面中的纸翼变湿后会染上蓝灰色，角色上方和 HUD 显示湿度。展翼有纸面遮雨范围，雨滴会在屋檐与护翼处停止。
- 界面使用和纸、墨色、朱印与日式檐亭；飞行振翅幅度从约 9° 增至约 52°，地面与滑翔各有不同幅度，翅根作为旋转中心。
...
25 项单元测试覆盖四关的真实操作通关路线（无传送）、跳跃滑翔、两轴碰撞、叠站和折桥、个人存档、各人数机关与全员过关、移动平台、输入校验与房间权限，以及湿度、屋檐恢复、互相挡雨、湿透回存档、旧房间兼容与机关蓄力。另有 3 项真实 WebSocket 集成测试覆盖 2 / 3 / 6 人同步、座位隔离、视角共享、展翼状态、掉线暂停和重连。

## 结构

- `src/game.ts`：确定性 60 Hz 物理与四个手工关卡。
- `src/weather.ts`：雨区、檐亭、雨势与遮雨半径。
